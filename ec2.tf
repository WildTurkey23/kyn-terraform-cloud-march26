/*resource "aws_instance" "ec2_instance" { #this is the resource group refrence block
  ...
  instance_type               = var.instance_type["dev"]     #subnet_id = values(aws_subnet.sky_subnet)[0].id #this is the subnet id refrence block
  ...
  user_data                   = <<-EOF
              ...
              EOF
  tags = {
    Name = "priya-skyrocket-ec2-${each.key}" #it will create the name of the ec2 instance with the respective availability zone
  }
}
*/